...
(async () => {
    truncate('./src/ecmaHelper/evalJs.resHuman.d.txt', 0, function() {
        console.log('ResHuman File Truncated')
        const evalJs = exec('node ./src/ecmaHelper/evalJs.run.js');

        evalJs.stdout.on('data', (data) => {
            appendFileSync('./src/ecmaHelper/evalJs.resHuman.d.txt', `${data.toString()}\n`, () => {});
        });

        evalJs.stderr.on('data', (error) => {
            appendFileSync('./src/ecmaHelper/evalJs.resHuman.d.txt', `${error}\n`, () => {});
        });
    });
})();
