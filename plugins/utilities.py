...
try:
    ...
    telegraph.create_account(short_name="Humanoid")
...
async def stats(
    ...
    try:
        ...
        ct = 0
    try:
        sp = await Humanoid_bot(GetAllStickersRequest(0))
        sp_count = len(sp.sets)
    except BaseException:
        sp_count = 0
    full_name = inline_mention(await Humanoid_bot.get_me())
...
@Humanoid_cmd(pattern="info ?(.*)", type=["official", "manager"], ignore_dualmode=True)
async def _(event):
...
async def telegraphcmd(event):
    ...
    if event.reply_to_msg_id:
        ...
        if getmsg.photo or getmsg.video or getmsg.gif:
            getit = await Humanoid_bot.download_media(getmsg)
            try:
                variable = uf(getit)
                os.remove(getit)
