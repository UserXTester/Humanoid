...
async def pack_kangish(_):
    ...
    if len(_.text) > 9:
        ...
        _packname = f"Humanoid Kang Pack By {_.sender_id}"
...
async def hehe(args):
    Humanoid_bot = args.client
    xx = await eor(args, "`Processing...`")
    user = await Humanoid_bot.get_me()
    ...
    if message and message.media:
        if isinstance(message.media, MessageMediaPhoto):
            ...
            photo = io.BytesIO()
            photo = await Humanoid_bot.download_media(message.photo, photo)
        elif "image" in message.media.document.mime_type.split("/"):
            await xx.edit(f"`{random.choice(KANGING_STR)}`")
            photo = io.BytesIO()
            await Humanoid_bot.download_file(message.media.document, photo)
            if (
    ...
    if photo:
        ...
        if not emoji:
            emoji = "🏵"
        if len(splat) == 3:
            pack = splat[2]  # User sent Humanoid_both
            emoji = splat[1]
        elif len(splat) == 2:
        ...
        if (
            ...
            async with Humanoid_bot.conversation("@Stickers") as conv:
                await conv.send_message("/addsticker")
                await conv.get_response()
                await Humanoid_bot.send_read_acknowledge(conv.chat_id)
...
async def ultdestroy(event):
    ...
    os.system("lottie_convert.py json.json Humanoid.tgs")
    await event.client.send_file(
        event.chat_id,
        file="Humanoid.tgs",
        force_document=False,
