...
@Humanoid_cmd(pattern="superfban ?(.*)", ignore_dualmode=True)
async def _(event):
        ...
        for a in range(3):
            async with event.client.conversation("@MissRose_bot") as bot_conv:
                ...
                if "make a file" in response.text or "Looks like" in response.text:
                    ...
                    await asyncio.sleep(3)
                    if fedfile.media:
                        downloaded_file_name = await Humanoid.download_media(
                            fedfile,
                            "fedlist",
                        )
    ...
    try:
        await Humanoid.send_message(chat, f"/start")
    except BaseException:
...
@Humanoid_cmd(pattern="superunfban ?(.*)", ignore_dualmode=True)
async def _(event):
    ...
    for fed in fedList:
        if udB.get("EXCLUDE_FED") and fed in excludeFed:
            ...
            exCount += 1
            continue
        await Humanoid.send_message(chat, f"/joinfed {fed}")
        await asyncio.sleep(3)
        await Humanoid.send_message(chat, f"/unfban {FBAN} {REASON}")
        await asyncio.sleep(3)
    try:
        os.remove("fedlist")
    except Exception as e:
...
@Humanoid_cmd(pattern="fstat ?(.*)", ignore_dualmode=True)
async def _(event):
    ...
    if sysarg == "":
        ...
        async with event.client.conversation(bot) as conv:
            try:
                ...
                await Humanoid.send_read_acknowledge(bot)
            except YouBlockedUserError:
                await ok.edit("**Error**\n `Unblock` @MissRose_Bot `and try again!")


@Humanoid_cmd(pattern="fedinfo ?(.*)", ignore_dualmode=True)
async def _(event):
