...
@Humanoid_cmd(pattern="pin ?(.*)", type=["official", "manager"], ignore_dualmode=True)
async def pin(msg):
...
@Humanoid_cmd(pattern="purge ?(.*)", type=["official", "manager"], ignore_dualmode=True)
...
async def fastpurgerme(purg):
    ...
    async for msg in purg.client.iter_messages(
        ...
        msgs.append(purg.reply_to_msg_id)
        if len(msgs) == 100:
            await Humanoid_bot.delete_messages(chat, msgs)
            msgs = []
