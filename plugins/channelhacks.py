...
from pyHumanoid.functions.ch_db import *
    ...
    for ys in y:
        try:
            if e.text and not e.media:
                await Humanoid_bot.send_message(int(ys), e.text)
            elif e.media and e.text:
                await Humanoid_bot.send_file(int(ys), e.media, caption=e.text)
            else:
                await Humanoid_bot.send_file(int(ys), e.media)
        except Exception as e:
            await Humanoid_bot.send_message(bot.me.id, str(e))


@Humanoid_cmd(pattern="shift (.*)")
async def _(e):
...
@Humanoid_cmd(pattern="listsource")
...
async def list_all(event):
    ...
    for channel in channels:
        name = ""
        try:
            name = (await Humanoid.get_entity(int(channel))).title
        except BaseException:
            name = ""
        msg += f"=> **{name}** [`{channel}`]\n"
