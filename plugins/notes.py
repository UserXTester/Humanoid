...
from pyHumanoid.functions.notes_db import *
...
@Humanoid_cmd(pattern="addnote ?(.*)", admins_only=True)
async def an(e):
...
@Humanoid_cmd(pattern="remnote ?(.*)", admins_only=True)
async def rn(e):
    wrd = (e.pattern_match.group(1)).lower()
    chat = e.chat_id
    if not wrd:
...
@Humanoid_cmd(pattern="listnote$", admins_only=True)
async def lsnote(e):
