...
@Humanoid_cmd(pattern="listnm$")
async def rem_grp(e):
    chats = night_grps()
    name = "NightMode Groups Are-:\n\n"
    for x in chats:
        try:
            ok = await Humanoid_bot.get_entity(x)
            if ok.username:
                name += "@" + ok.username
            else:
                name += ok.title
...
async def open_grp():
    ...
    for chat in chats:
        try:
            ...
            await Humanoid_bot.send_message(chat, "**NightMode Off**\n\nGroup Opened 🥳.")
