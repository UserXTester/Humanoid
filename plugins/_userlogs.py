...
async def all_messages_catcher(e):
    ...
    send = await Humanoid_bot.get_messages(e.chat_id, ids=e.id)
    try:
        if x.username:
        ...
        await Humanoid_bot.send_message(NEEDTOLOG, "Please Give Your Assistant Bot")
...
async def _(e):
    ...
    y = await Humanoid_bot.get_entity(int(wah))
...
async def leave_ch_at(event):
    ...
    if client == "bot":
        ...
        name = (await Humanoid_bot.get_entity(int(ch_id))).title
        await Humanoid_bot.delete_dialog(int(ch_id))
