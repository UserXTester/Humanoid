...
@Humanoid_cmd(pattern="fsub ?(.*)", admins_only=True, groups_only=True)
async def addfor(e):
...
@Humanoid_cmd(pattern="checkfsub$")
async def getfsr(e):
...
async def diesoon(e):
    ...
    try:
        await Humanoid_bot(GetParticipantRequest(int(spli[1]), int(spli[0])))
