...
from pyHumanoid.functions.blacklist_db import *

from . import *


@Humanoid_cmd(pattern="blacklist ?(.*)", admins_only=True)
async def af(e):
...
@Humanoid_cmd(pattern="remblacklist ?(.*)", admins_only=True)
async def rf(e):
    wrd = e.pattern_match.group(1)
    chat = e.chat_id
    if not wrd:
...
@Humanoid_cmd(pattern="listblacklist$", admins_only=True)
async def lsnote(e):
