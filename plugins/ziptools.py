...
async def zipp(event):
    ...
    await event.client.send_file(
        ...
        xxx,
        force_document=True,
        thumb="resources/extras/Humanoid.jpg",
        caption=f"`{xxx.name}`",
        reply_to=reply,
    )
    os.remove(inp)
...
async def do_zip(event):
    ...
    xx = await eor(event, "`processing`")
    await bash(f"zip -r Humanoid.zip zip/*")
    k = time.time()
    xxx = await uploader("Humanoid.zip", "Humanoid.zip", k, xx, "Uploading...")
    await event.client.send_file(
        event.chat_id,
        xxx,
        force_document=True,
