...
async def _(e):
    ...
    if put:
        try:
            result = await e.client.inline_query(asst.me.username, f"msg {put}")
        except rep:
            return await eor(
                e,
        ...
        await result[0].click(e.chat_id, reply_to=e.reply_to_msg_id, hide_via=True)
        await e.delete()
    else:
...
async def _(e):
    ...
    if "wspr" not in vvv:
        try:
            logi = await Humanoid_bot(gu(id=query))
        ...
        try:
            logi = await Humanoid_bot.get_entity(query)
            button = [
