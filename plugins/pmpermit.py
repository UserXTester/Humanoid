...
from pyHumanoid.functions.logusers_db import *
from pyHumanoid.functions.pmpermit_db import *
...
if Redis("PMPIC"):
    ...
    PMPIC = "resources/extras/teamHumanoid.jpg"
...
if sett == "True":
    ...
    async def autoappr(e):
        ...
        if not is_approved(e.chat_id):
            ...
            try:
                await Humanoid_bot.edit_folder(e.chat_id, folder=0)
            except BaseException:
                pass
    ...
    async def permitpm(event):
        ...
        if not apprv and event.text != UND:
            if Redis("MOVE_ARCHIVE") == "True":
                try:
                    await Humanoid.edit_folder(user.id, folder=1)
                except BaseException:
                    pass
            if event.media:
                await event.delete()
            ...
            if user.id in LASTMSG:
                ...
                if event.text != prevmsg:
                    ...
                    if inline_pm == "False":
                        ...
                        try:
                            result = await Humanoid.inline_query(
            ...
            if COUNT_PM[user.id] >= WARNS:
                ...
                await Humanoid(BlockRequest(user.id))
                await Humanoid(ReportSpamRequest(peer=user.id))
...
@Humanoid_cmd(pattern="block ?(.*)", ignore_dualmode=True)
...
@Humanoid_cmd(pattern="unblock ?(.*)", ignore_dualmode=True)
...
async def apr_in(event):
    ...
    if not is_approved(uid):
        approve_user(uid)
        try:
            await Humanoid_bot.edit_folder(uid, folder=0)
        except BaseException:
            pass
        try:
            user_name = (await Humanoid.get_entity(uid)).first_name
        except BaseException:
            user_name = ""
        await event.edit(
...
async def blck_in(event):
    uid = int(event.data_match.group(1).decode("UTF-8"))
    await Humanoid(BlockRequest(uid))
    try:
        user_name = (await Humanoid.get_entity(uid)).first_name
    except BaseException:
        user_name = ""
...
async def unblck_in(event):
    uid = int(event.data_match.group(1).decode("UTF-8"))
    await Humanoid(UnblockRequest(uid))
    try:
        user_name = (await Humanoid.get_entity(uid)).first_name
    except BaseException:
        user_name = ""
...
async def ytfuxist(e):
    try:
        ...
        try:
            await Humanoid.delete_messages(e.chat_id, e.id)
        except BaseException:
            pass
...
async def delete_pm_warn_msgs(chat: int):
    async for i in Humanoid_bot.iter_messages(chat, from_user="me"):
        tx = i.text
