...
from pyHumanoid.functions.gcast_blacklist_db import *
...
async def _(e):
    ...
    if user:
        ...
        if user.isdigit():
            user = int(user)
        try:
            name = await Humanoid_bot.get_entity(user)
        except BaseException:
            return await eod(e, f"`No User Found Regarding {user}`")
        ...
        rank = "Not AdMin"
        c = 0
        async for x in Humanoid_bot.iter_dialogs():
            if "group" in key.lower():
                if x.is_group:
                    try:
                        await Humanoid_bot(
        ...
        if x.is_group:
            ...
            if not is_gblacklisted(chat):
                try:
                    ...
                    await Humanoid_bot.send_message(chat, msg)
                except BaseException:
                    er += 1
...
async def list_gengbanned(event):
    ...
    for i in users:
        try:
            name = (await Humanoid.get_entity(int(i))).first_name
        except BaseException:
            name = i
        msg += f"**User**: {name}\n"
...
@Humanoid_cmd(pattern="gblacklist")
async def blacklist_(event):
    await gblacker(event, "add")


@Humanoid_cmd(pattern="ungblacklist")
...
async def gblacker(event, type_):
    ...
    try:
        ...
        pass
    try:
        chat_id = (await Humanoid.get_entity(chat)).id
