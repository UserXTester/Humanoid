...
from pyHumanoid.functions.warn_db import *
from telethon.utils import get_display_name

from . import *


@Humanoid_cmd(pattern="warn ?(.*)", groups_only=True, admins_only=True)
async def warn(e):
    ...
    if reply:
        ...
        try:
            user = e.text.split()[1]
            if user.startswith("@"):
                ok = await Humanoid_bot.get_entity(user)
                user = ok.id
            else:
                user = int(user)
    ...
    if count + 1 >= number:
        if "ban" in action:
            ...
            try:
                await Humanoid_bot.kick_participant(e.chat_id, user)
            except BaseException:
                return await eod(e, "`Something Went Wrong.`")
        elif "mute" in action:
            try:
...
@Humanoid_cmd(pattern="resetwarn ?(.*)", groups_only=True, admins_only=True)
async def rwarn(e):
    reply = await e.get_reply_message()
    if reply:
        user = reply.from_id.user_id
...
@Humanoid_cmd(pattern="warns ?(.*)", groups_only=True, admins_only=True)
async def twarns(e):
    reply = await e.get_reply_message()
    if reply:
        user = reply.from_id.user_id
