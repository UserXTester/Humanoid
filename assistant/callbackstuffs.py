...
# --------------------------------------------------------------------#
telegraph = Telegraph()
r = telegraph.create_account(short_name="Humanoid")
auth_url = r["auth_url"]
# --------------------------------------------------------------------#
...
async def update(eve):
    ...
    if Var.HEROKU_API:
        ...
        execl(sys.executable, sys.executable, "-m", "pyHumanoid")
...
async def changes(okk):
    ...
    if len(changelog_str) > 1024:
        ...
        with open(f"Humanoid_updates.txt", "w+") as file:
            file.write(tl_chnglog)
        await okk.edit(
            get_string("upd_5"),
            file="Humanoid_updates.txt",
            buttons=Button.inline("Update Now", data="updatenow"),
        )
        remove(f"Humanoid_updates.txt")
        return
    else:
        await okk.edit(
            changelog_str,
...
async def otvaar(event):
    await event.edit(
        "Other Variables to set for @TheHumanoid:",
        buttons=[
            [
